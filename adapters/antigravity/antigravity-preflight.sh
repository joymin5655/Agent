#!/usr/bin/env bash
# antigravity-preflight.sh — fail-closed health probe for the Antigravity (agy) worker lane.
#
...
#
# usage: antigravity-preflight.sh [lane]           # lane = a backends.json backend name
...
PROBE_TIMEOUT_S="${ANTIGRAVITY_PREFLIGHT_TIMEOUT_S:-60}"
PROBE_TOKEN="ANTIGRAVITY-PREFLIGHT-OK-7d3f1a"
...
[[ "$PROBE_TIMEOUT_S" =~ ^[0-9]+$ ]] || {
    echo "antigravity-preflight: ANTIGRAVITY_PREFLIGHT_TIMEOUT_S is not numeric ('$(sanitize "$PROBE_TIMEOUT_S")') — refusing" >&2
    exit 7
}
command -v jq >/dev/null 2>&1 || {
    echo "antigravity-preflight: jq is required to resolve lane '$(sanitize "$LANE")' from the backends registry — refusing rather than guessing the dispatch argv" >&2
    exit 7
}
[[ -f "$REGISTRY" ]] || {
    echo "antigravity-preflight: backends registry not found: $(sanitize "$REGISTRY") — refusing" >&2
    exit 7
}
...
[[ -n "$CLI" ]] || {
    echo "antigravity-preflight: lane '$(sanitize "$LANE")' yields no cmd[0] from $(sanitize "$REGISTRY") — refusing" >&2
    exit 7
}
