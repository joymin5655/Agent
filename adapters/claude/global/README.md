...
Adopted: 2026-05-12 from a production reference setup and kept project-agnostic.
...
3. Sub-project (`<repo>/packages/*/CLAUDE.md`, `<repo>/apps/*/CLAUDE.md`, etc.)
...
```bash
mkdir -p ~/.claude
cp adapters/claude/global/CLAUDE.md ~/.claude/CLAUDE.md
cp adapters/claude/global/karpathy.md ~/.claude/karpathy.md
cp adapters/claude/global/RTK.md ~/.claude/RTK.md   # optional — RTK CLI is per-user setup
```
...
Project `CLAUDE.md` should **cross-reference** Karpathy principles rather than restate them. Generic pattern:

```markdown
## Agent Operating Mode

**Project-specific principles** (Karpathy global §3 Surgical / §4 Goal-Driven inherited):
1. Domain safety — state uncertainty and risk where relevant
2. Context efficiency — read the smallest authoritative doc set first
