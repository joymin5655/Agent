#!/usr/bin/env bash
# grok-preflight.sh — fail-closed health probe for the xAI grok worker lane.
#
...
#   (b) a cached credential (~/.grok/auth.json) can be stale — presence of the
...
#
# usage: grok-preflight.sh [lane]           # lane = a backends.json backend name
...
#        GROK_PREFLIGHT_TIMEOUT_S (default 60)
...
LANE="${1:-${AGENT_PREFLIGHT_LANE:-grok}}"
REGISTRY="${AGENT_BACKENDS_FILE:-$SELF_DIR/../../core/infra/backends.json}"
PROBE_TIMEOUT_S="${GROK_PREFLIGHT_TIMEOUT_S:-60}"
PROBE_TOKEN="GROK-PREFLIGHT-OK-9c2e4b"
...
[[ "$PROBE_TIMEOUT_S" =~ ^[0-9]+$ ]] || {
    echo "grok-preflight: GROK_PREFLIGHT_TIMEOUT_S is not numeric ('$(sanitize "$PROBE_TIMEOUT_S")') — refusing" >&2
    exit 7
}
command -v jq >/dev/null 2>&1 || {
    echo "grok-preflight: jq is required to resolve lane '$(sanitize "$LANE")' from the backends registry — refusing rather than guessing the dispatch argv" >&2
    exit 7
}
[[ -f "$REGISTRY" ]] || {
    echo "grok-preflight: backends registry not found: $(sanitize "$REGISTRY") — refusing" >&2
    exit 7
}
...
[[ -n "$CLI" ]] || {
    echo "grok-preflight: lane '$(sanitize "$LANE")' yields no cmd[0] from $(sanitize "$REGISTRY") — refusing" >&2
    exit 7
}
command -v "$CLI" >/dev/null 2>&1 || {
    echo "grok-preflight: worker '$(sanitize "$CLI")' (cmd[0] of lane '$(sanitize "$LANE")') not found on PATH — install: ln -sf .../adapters/grok/grok-worker.sh ~/bin/grok-worker" >&2
    exit 1
}

OUT="$(mktemp)" || { echo "grok-preflight: mktemp failed — refusing rather than probing blind" >&2; exit 6; }
...
emit_capture() { tr -d '\000-\010\013-\037\177' < "$OUT" | sed -e 's/^/grok-preflight:   /' >&2; }

if [[ $probe_rc -eq 137 ]]; then
    echo "grok-preflight: probe timed out after ${PROBE_TIMEOUT_S}s — refusing" >&2
    exit 4
fi
...
if grep -qiE 'not logged in|please log in|log ?in with|unauthorized|unauthenticated|forbidden|access denied|authentication failed|invalid.*(api.?key|credential|token)|(credential|token|session|login|subscription)[^.]{0,40}(expired|has expired)' "$OUT"; then
    echo "grok-preflight: the CLI reports an authentication failure — refusing" >&2
    emit_capture
    exit 3
fi
if [[ $probe_rc -ne 0 ]]; then
    echo "grok-preflight: probe exited $probe_rc — refusing (state unknown)" >&2
    emit_capture
    exit 5
fi
...
if ! sed -e 's/'$'\033''\[[0-9;?]*[a-zA-Z]//g' "$OUT" | grep -Fq "$PROBE_TOKEN"; then
    echo "grok-preflight: probe exited 0 but the reply does not contain the requested token ($PROBE_TOKEN) — no proof of inference, refusing" >&2
    emit_capture
    exit 5
fi
exit 0
