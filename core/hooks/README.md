...
| `circuit-breaker.py` | PostToolUse (Bash) | Detect repeated Bash failures + advise strategy change. Failure classification prefers a machine exit status (resolved by key PRESENCE, so a successful `0` is not discarded as falsy) and falls back to a narrowed text heuristic that scrubs zero-count phrasings (`0 errors`) — this runtime supplies no exit status, measured 2026-07-30 |
| `verify-observer.py` | PostToolUse (Bash) | Pure observer — record that a verification command was INVOKED this session (family label only: tests/battery/typecheck/lint/build; no command text, so no secret-leak surface). Read by `session-quality-gate.py` layer 3, which advises when the session diff carries code changes and nothing was ever run. Never claims a verification passed (no exit status available). Sink `.agent/logs/verify-observed.jsonl` |
...
| `model-routing-advisor.py` | PreToolUse (Task/Agent) | Decision-time advisory — one-line `additionalContext` nudge when a dispatch has no `model` override, isn't a registry-pinned specialist, and isn't `Plan` (Plan's inherit is intended). Never blocks, never switches a model, writes no log of its own — advisory only, the observer's sink stays the sole measured record |
| `session-tier-observer.py` | SessionStart | Pure observer — best-effort detect the session's model tier (stdin model field → transcript tail → settings default) and surface the routing advisory on stderr ("session=TOP; execution dispatches at MID/LOW"); logs to `.agent/logs/session-tier.jsonl` (audit trail; /manager-audit does not consume it yet — future wiring). Detection is allowed, switching is not (docs/model-routing.md) |
...
| `council-escalation-gate.py` | PreToolUse (Task/Agent) | Deny a plain `code-reviewer` dispatch when `core/infra/council-threshold.sh` judges the staged diff council-scale (line/file threshold or a risk-area path), pointing at `/council-review --staged`. Escapes: a council-active flag bound to the diff hash it authorized (TTL `AGENT_COUNCIL_ACTIVE_TTL_S`, default 300s — council-review's own internal dispatch is never self-blocked; set/cleared via `--council-flag set` / `--council-flag clear`, never a raw touch), and same-diff-hash single-deny (TTL `AGENT_COUNCIL_DENY_TTL_S`, default 3600s — a retry on the identical diff is let through with a stderr warning, not denied forever). Escape-hatch state lives outside the reviewed repo (`~/.agent/state/council` by default, `AGENT_COUNCIL_STATE_DIR` seam) so a hostile checkout can't forge or backdate it; the firing log stays in-repo (`security-violations.jsonl`) since it's write-only telemetry, never read back for a decision |
