...
# Reset legacy temp supervisor flags for older local installs. New installs keep
# state in `.agent-harness/state/`.
rm -f /tmp/agent-harness-intent-feature \
      /tmp/agent-harness-required-agents \
      /tmp/agent-harness-dispatched-agents \
      /tmp/agent-harness-mode \
      /tmp/agent-harness-supervisor-analysis.json 2>/dev/null || true
