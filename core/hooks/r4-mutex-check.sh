#!/usr/bin/env bash
# Agent Harness — PreToolUse R4 mutex check.
...
log_violation() {
  ...
  printf '{"ts":"%s","guard":%s,"hook":"r4-mutex-check.sh","resource":"%s","reason":%s,"session_id":"%s","decision":"deny"}\n' \
    "$ts" "$guard" "$resource" "$(printf '%s' "$reason" | python3 -c 'import sys,json; print(json.dumps(sys.stdin.read()))' 2>/dev/null || echo "\"$reason\"")" "$sid" \
