...
def _resolve_project_root() -> Path:
    """2026-05-18 W3 — portable PROJECT_ROOT.

    cwd 의 git toplevel 우선 (다른 프로젝트에서 hook 작동 가능),
    실패 시 __file__ parents[2] fallback (AirLens 메인 트리 보장).
    timeout 2s — git 부재 / network FS 지연 시 fallback.
    """
    try:
        import subprocess
        result = subprocess.run(
            ["git", "rev-parse", "--show-toplevel"],
            cwd=Path.cwd(), capture_output=True, text=True, timeout=2,
        )
...
AGENT_ALIASES: dict[str, tuple[str, ...]] = {
    ...
    # 2026-05-18 W2 wave-2 — Wave 1 baseline 기반 추가 (8 row, 21 → 29).
    # 글로벌 102 agent 중 실제 호출된 14 specialist 매핑 (agent-routing.jsonl all-time).
    "claude-code-guide":       ("doc-writer",),
    "build-error-resolver":    ("edge-fn-dev", "fe-architect"),
    "copy-humanizer":          ("doc-writer",),
    "design-crawler":          ("ui-ux-director", "component-builder"),
    "healthcare-reviewer":     ("ml-security-reviewer",),
    "gsd-debugger":            ("test-engineer", "style-reviewer"),
    "vercel:performance-optimizer": ("performance-reviewer",),
    "tdd":                     ("test-engineer",),
}
...
# 2026-05-18 — Soft enforcement (PreToolUse Agent matcher): intent ∈ 본 set
# 이면 ask 안 함. SIMPLE_EDIT/META/QUERY/RECALL/LEARN 는 specialist dispatch
# 자체가 의미 없는 영역 (advisory § _maybe_print_advisory 와 같은 set).
# 진단 근거: ~/.claude/plans/airlens-tender-taco.md (271 records, conversion 0%).
_ASK_SKIP_INTENTS: frozenset[str] = frozenset({
    "SIMPLE_EDIT", "META", "QUERY", "RECALL", "LEARN",
})

QUERY_RE = re.compile(
    # 2026-05-18 W2 — Wave 1 baseline 분석 (829 SIMPLE_EDIT fallback) 후 +6 keyword.
    # "뭐가" / "조사" / "investigate" / "research" 추가.
    r"(뭐야|뭔가요|뭐가|알려줘|설명|어떻게|왜\s|보여줘|찾아줘|찾고|검색|조사|조사해|"
    r"what\b|how\b|why\b|show\b|find\b|explain\b|investigate\b|research\b)",
    re.IGNORECASE,
)
...
FEATURE_RE = re.compile(
    # 2026-05-18 W2 — Wave 1 baseline 분석 (829 SIMPLE_EDIT fallback) 후 +9 keyword.
    # "진행" 이 fallback 최대 빈도 (sample 4/15). "자동화/머지/푸시/배포" 누락 보강.
    r"(구현|추가|만들|생성|수정|변경|개선|리팩|통합|연동|빌드|작성|연결|도입|"
    r"최적화|진행|자동화|머지|푸시|배포|"
    r"implement|create|build|add|fix|refactor|integrate|change|optimi[sz]e|"
    r"merge|push|deploy|automate)",
    re.IGNORECASE,
)
REVIEW_RE = re.compile(
    # 2026-05-18 W2 — Wave 1 baseline 분석 후 +4 keyword.
    # "비교/정리/요약" — meta 작업이 SIMPLE_EDIT 로 fallback 되던 패턴 보강.
...
AGENT_PATTERNS: dict[str, re.Pattern[str]] = {
    ...
    "aq-data-analyst": re.compile(
        # 2026-05-18 — 옵션 E 키워드 확장 (8 → 32). 발의 = airlens-tender-taco.md
        # 진단 (271 records / aq-data-analyst 권고 0회 / 사용 패턴 영역 회피).
        r"(PM2\.5|PM10|PM1\b|초미세먼지|미세먼지|황사|"
        r"O3|NO2|SO2|CO\b|오존|이산화질소|이산화황|일산화탄소|"
        r"AQI|CAI|대기질|대기오염|공기질|air\s*quality|"
        r"IDW|크리깅|kriging|보간|interpolation|"
        r"AirKorea|OpenAQ|WAQI|측정소|"
        r"월경성|cross[-\s]border|transboundary|"
        r"spatial\s*pattern|spatiotemporal|시공간\s*분석)",
        re.IGNORECASE,
    ),
    ...
    # 2026-05-14 wiki-curator regex removed — phantom agent (orphan, agent file absent).
    # wiki life cycle 은 5 자산 (wiki-synth / wiki-auto-index / wiki-supersede-apply /
    # wiki-log-rotate / firecrawl-wiki-ingest) 이 이미 자동 커버.
    # See `.claude/rules/policy/wiki-automation-coverage.md` §History.
...
def emit_hint(analysis: dict[str, Any], index: RegistryIndex) -> dict[str, str]:
    ...
    header = (
        f"SUPERVISOR v6 [STRONG ADVISORY] | intent={intent} | risk={analysis['risk']} | "
    ...
    if intent in {"FEATURE", "MULTI_DEPT"}:
        ...
        body.append("**REQUIRED specialist**: " + ", ".join(agents[:3]))
        body.append(f"Dispatch: Agent(subagent_type='{agents[0]}', prompt='[question or audit scope]')")
        body.append(
            "If NOT dispatching specialist, state explicit reject reason: "
            "(a) agent file absent (verify `ls ~/.claude/agents/<name>.md`) / "
            "(b) simpler tool (Read/Grep) sufficient / "
            "(c) user explicit fallback. "
            "Silent fallback to Explore/general-purpose = advisory-ignored pattern "
            "(98% skew, feedback_supervisor_advisory_systematically_ignored.md)."
        )
...
def _maybe_print_advisory(analysis: dict[str, Any]) -> None:
    """2026-05-18 W2 T3.2 — LOW/MEDIUM risk advisory.

    handle_pre_tool_use 의 early-exit (intent != MULTI_DEPT and risk != HIGH) 직전 호출.
    matched_agents 매핑 있고 dispatched 부족 시 stderr 1줄 hint. block X / fail-safe.

    Wave 1 baseline: 98% matched-invoked skew (obs 2929) — high-risk 외 specialist
    추천이 advisory 조차 없던 갭 보강.
    """
    try:
        intent = analysis.get("intent", "")
        if intent in {"SIMPLE_EDIT", "META", "QUERY", "RECALL", "LEARN"}:
            return
        matched = analysis.get("matched_agents") or []
        if not matched:
            return
        dispatched = _expand_dispatched(_load_json_list(DISPATCHED_FLAG))
        missing = [a for a in matched if a not in dispatched]
        if not missing:
            return
        sugg: list[str] = []
        for spec in missing[:3]:
            sugg.append(spec)
            for global_agent in _REVERSE_ALIASES.get(spec, [])[:1]:
                sugg.append(f"{global_agent}*")
        if sugg:
            print(
                f"SUPERVISOR HINT: specialist 추천 — {', '.join(sugg)} "
                f"(intent={intent}, risk={analysis.get('risk', '?')}, * = 글로벌 alias)",
                file=sys.stderr,
            )
    except Exception:
        pass  # fail-safe — advisory 실패가 hook 자체 차단으로 이어지면 안 됨


def _should_ask_for_specialist(
    subagent_type: str,
    analysis: dict[str, Any],
    dispatched: set[str],
) -> tuple[bool, list[str]]:
    """Generic agent 호출 + specialist 권고 미커버 → ask 결정.

    Returns (should_ask, uncovered_recommendations) — should_ask=True 면 list 비공.
    NON_SPECIALIST_AGENTS frozenset (Explore/Plan/general-purpose/code-explorer)
    재사용. intent ∈ _ASK_SKIP_INTENTS 또는 matched 빈 list 면 skip.
    """
    if subagent_type not in NON_SPECIALIST_AGENTS:
        return False, []  # specialist 직접 호출은 통과
    intent = analysis.get("intent", "")
    if intent in _ASK_SKIP_INTENTS:
        return False, []
    matched = analysis.get("matched_agents") or []
    if not matched:
        return False, []
    if subagent_type in matched:
        return False, []  # generic 이 권고 list 에 포함된 edge case
    uncovered = [a for a in matched if a not in dispatched]
    return (len(uncovered) > 0, uncovered)


def _log_ask_decision(subagent_type: str, analysis: dict[str, Any], uncovered: list[str]) -> None:
    """Evidence sink — .claude/logs/agent-ask-decisions.jsonl append-only."""
    try:
        from datetime import datetime
        log_path = PROJECT_ROOT / ".claude" / "logs" / "agent-ask-decisions.jsonl"
        ...
        record = {
            ...
            "matched_agents": analysis.get("matched_agents") or [],
            "uncovered": uncovered,
        }
...
def _handle_pre_tool_use_agent(input_data: dict[str, Any]) -> None:
    """Agent tool 호출 직전 — generic + specialist 권고 미커버 시 ask.

    `_should_ask_for_specialist` 가 True 반환할 때만 permissionDecision="ask".
    그 외 = pass (빈 stdout). fail-safe — exception 시 pass.

    진단 근거: ~/.claude/plans/airlens-tender-taco.md (271 records, conversion 0%
    across all 18 AirLens domain agents, match rate 2.3%).
    """
    try:
        ...
        dispatched = _expand_dispatched(_load_json_list(DISPATCHED_FLAG))
        should_ask, uncovered = _should_ask_for_specialist(subagent_type, analysis, dispatched)
        if not should_ask:
            return

        suggestions: list[str] = []
        for spec in uncovered[:3]:
            suggestions.append(f'subagent_type="{spec}" (native specialist)')
            for global_agent in _REVERSE_ALIASES.get(spec, [])[:1]:
                suggestions.append(f'subagent_type="{global_agent}" (alias → {spec})')

        intent = analysis.get("intent", "?")
        risk = analysis.get("risk", "?")
        reason_lines = [
            f"SUPERVISOR ASK — generic agent '{subagent_type}' 호출 감지 (intent={intent}, risk={risk}).",
            f"권고된 specialist: {', '.join(uncovered)}",
            "",
            "선택지:",
            "  1) 권고 specialist 로 재호출 (예시 아래)",
            "  2) 현재 generic 호출 유지 (사용자 명시 승인)",
            "",
            "권고 invocation:",
        ]
        for sugg in suggestions:
            reason_lines.append(f"  - {sugg}")
        reason_lines.append("")
        reason_lines.append("근거: ~/.claude/plans/airlens-tender-taco.md (271 records / conversion 0% / match 2.3%)")
        ...
        _log_ask_decision(subagent_type, analysis, uncovered)
        print(json.dumps(payload, ensure_ascii=False))
    except Exception:
        # fail-safe — ask 결정 실패가 Agent 호출 자체 차단으로 이어지면 안 됨
        return
...
def handle_pre_tool_use(input_data: dict[str, Any]) -> None:
    ...
    # 2026-05-18 — Agent matcher: soft enforcement via ask.
    if tool_name == "Agent":
        _handle_pre_tool_use_agent(input_data)
        return
    ...
    if intent != "MULTI_DEPT" and risk != "HIGH":
        _maybe_print_advisory(analysis)  # W2 T3.2 — LOW/MEDIUM risk advisory
        return
