#!/usr/bin/env bash
# call-worker.sh — cross-vendor second-opinion dispatcher.
#
# Resolves a role from core/infra/backends.json (role -> backend -> CLI argv),
# streams the prompt from stdin to the backend CLI, and captures the reply to
# .agent/workers/<utc-ts>-<role>.md (path printed on stdout). Model names never
# appear here or in backends.json — each vendor's adapter/profile owns its tier
# (docs/model-routing.md § Cross-vendor second-opinion lane).
#
...
# run_backend <name> — 0 ok, 124 timeout, 127 CLI missing, else CLI's exit.
...
if [[ $rc -eq 127 ]]; then
    if [[ -n "$FALLBACK_REASON" ]]; then
        echo "call-worker: no backend available for '$ROLE' — $FALLBACK_REASON, and fallback '$FALLBACK' CLI not found either. Install one of them." >&2
    else
