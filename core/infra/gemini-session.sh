...
#   1. start lock entry + create worktree (.worktrees/gemini-<slug>)
#   2. spawn background heartbeat loop (every 5 min)
#   3. cd into worktree and exec gemini
#   4. on exit: kill heartbeat + release lock
#
# Worktree is NOT auto-removed; use `git worktree remove .worktrees/gemini-<slug>` afterward.
...
cleanup() {
  ...
  AGENT=gemini AGENT_SESSION_ID="$AGENT_SESSION_ID" \
