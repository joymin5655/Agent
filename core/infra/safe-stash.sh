#!/usr/bin/env bash
# Safe stash — persistent backup for blocking-untracked files during rebase / worktree ops.
#
# Why: when `git rebase` blocks on an untracked file, moving it to /tmp is unsafe
# (system cleanup can wipe /tmp at any time). Use git stash --include-untracked OR
# this helper, which stores snapshots under ~/.agent/backup/.
#
# Usage:
#   safe-stash.sh save <branch-slug>           # snapshot all untracked files
...
#   safe-stash.sh list                         # list snapshots, newest first
#   safe-stash.sh prune <days>                 # delete snapshots older than N days
#
# Storage:
#   ~/.agent/backup/YYYY-MM-DD-HHMMSS-<slug>/...
#   Override with $SAFE_STASH_ROOT env var.

set -euo pipefail

BACKUP_ROOT="${SAFE_STASH_ROOT:-$HOME/.agent/backup}"
...
ENV:
  SAFE_STASH_ROOT   override backup root (default ~/.agent/backup)
USAGE
  exit 2
}
...
cmd_save() {
  ...
  echo "snapshot -> $dest"
  echo "  restore: core/infra/safe-stash.sh restore $slug"
}
