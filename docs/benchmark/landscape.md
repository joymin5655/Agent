...
- **L2 provenance/license ledger** — citypaul/.dotfiles vendors skills from
  ...
  imports, not building a catalog). Done-condition sketch — an illustrative
  ledger path (not yet created; fenced so the doc-reality gate treats it as
  an example, not a claim):
  ```
  docs/l2-provenance.md
  ```
  (or an equivalent name) plus a doc-reality-style check that flags an
  L2-sourced file with no provenance entry.
