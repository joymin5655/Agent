...
| `check-hardcoding.py` | No — hardcoded in-script (patterns/exempts; wiring = backlog T-4) | Flags hardcoded design literals. Default **dryrun** (advisory + `.agent/logs/hardcoding.jsonl`); `AGENT_HARDCODING_MODE=off\|dryrun\|block` (block restores the pre-2026-07 deny) |
