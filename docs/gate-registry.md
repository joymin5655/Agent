...
GATE completion-gate | completion-gate.sh | block(opt-in; default=dryrun) | completion-gate.jsonl | * | 2026-08-20 | Model claims done without deterministic evidence (false success 45-76%, arXiv 2606.09863; judge-only blocking overtrusts, arXiv 2406.07791)
GATE council-escalation | council-escalation-gate.py | deny | security-violations.jsonl | council-escalation | 2026-08-21 | A council-scale diff (line/file threshold or a risk-area path, core/infra/council-threshold.sh) gets a plain Claude-solo code-reviewer dispatch instead of a multi-vendor /council-review, because the routing decision was left to a prompt suggestion nobody read — the same ~98% advisory-ignore rate supervisor.py's own dispatcher was built to correct. Escapes: council-review's own internal code-reviewer dispatch via a council-active flag bound to the diff hash it authorized (TTL AGENT_COUNCIL_ACTIVE_TTL_S, default 300s; state lives outside the reviewed repo, never trusted from inside it) and same-diff-hash single-deny (TTL AGENT_COUNCIL_DENY_TTL_S, default 3600s; a retry on the identical diff is let through with a stderr warning, so a caller that truly cannot run council-review is not stuck in a permanent deny loop). 2026-08-21 security revision: escape-hatch state (flag, deny ledger) moved outside the repo and bound/timestamped so a hostile checkout can no longer forge or permanently seed a bypass; the firing log stays in-repo (write-only telemetry, never read back for a decision) so it is now measured here rather than UNINSTRUMENTED.
<!-- gate-registry:end -->
