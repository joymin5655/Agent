...
1. **산출물 카운트(SSOT — `doc-reality.sh`의 (C) 체크가 라이브 대조):** `ls core/hooks/*.py core/hooks/*.sh | wc -l` = **25**(`core/hooks`의 .py+.sh 파일; `hook_config.py`·`trust_tier.py` 공용 모듈, `agent-inventory.py` 리컨사일러, `brain-capture.py` 세션 캡처 훅 포함), `ls core/tests/*.sh | wc -l` = **51**, `ls agents/*.md | wc -l` = **2**, `ls skills/*/SKILL.md | wc -l` = **8**(brain-ingest·harness-audit·harness-help·manager-audit·spec·supervise·verify-completion·wrap). §3.3 표의 훅·테스트 수치는 2026-07-04 스냅샷이며, 이후 증가분은 이 줄이 정본이다.
