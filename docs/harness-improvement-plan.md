...
| W-2 ✅ 2026-07-11 | `/reorg-sync` 스킬 — 이전/신규 경로 접두어를 인자로 받아 고아 참조 일괄 스윕(앵커·crontab·셔뱅·worktree gitfile·네이티브 메모리 키) | 드라이브 재구성마다 수작업 스윕 반복 | 픽스처 트리에서 5종 참조 전부 검출·치환 리포트. ✅ 실측(2026-07-11): `core/infra/reorg-sync.sh`(제네릭 — old/new 접두어·root 인자, 하드코딩 0; 5클래스 분류 후 DRY-RUN 리포트 기본·`--apply` 시 python str.replace 안전 리터럴 치환; native-memory-key는 `/._`→`-` 인코딩 변환 적용; `.git` 오브젝트스토어·바이너리·심링크 스킵하되 `.git` 워크트리 *파일*은 스윕; bare `/`·빈 old 거부) + `skills/reorg-sync/SKILL.md`(NOT절 2종=`mv` 아님·임의 텍스트치환 아님; report-then-confirm; ~/.claude·라이브 crontab은 tree 밖=참조만 치환·직접 미변경 명시) + `core/tests/reorg-sync-test.sh`(**72체크**: 5클래스 검출·dry무변경·5클래스 apply·인코딩 변환·idempotence·무매치 summary 0·`.git`스토어/바이너리/심링크 스킵·6 footgun 가드 + 경계 디코이 6종·NEW⊃OLD 수렴·dotted-key 인코딩·unwritable 보고·exec-bit 보존). registry-drift PASS(NOT절)·verify-all 자동편입. **적대 재리뷰(2026-07-15, 3레인 — 최초 리뷰 세션한도 사망=false-clean 판정이 정확했음)**: CRITICAL 2(무경계 치환의 형제 경로/키 오염·NEW⊃OLD 비멱등 누적) + MAJOR 2(비원자 쓰기 부분적용·손실 인코딩 kebab 충돌) 수정 — 경로 치환 경계앵커(`(?![A-Za-z0-9._-])`)·키 치환 `claude/projects` 라인 국한+양측 경계·NEW-보호 논스(멱등 보장)·tmp+rename 원자 쓰기(퍼미션 보존)+실패 파일 보고 exit1·개행 접두어 거부·`@keyword` cron 분류; 배터리 mutation 2라운드+보안 재리뷰 3라운드가 갭 4계열 적발→전부 픽스처 커버(dotted/underscore 인코딩·`/`-연속 논스-단독-방어·**ASCII-only 경계의 CJK/구두점 형제 누출=whitelist 반전으로 근본수정**·path 좌-경계 부재=무관 상위경로 꼬리 오염 봉쇄·**적대 Workflow 6렌즈 패널이 양 리뷰 레인 놓친 MAJOR 2건 적발=키레이어 whitelist화(`~+@%`·U+30FB 인코딩 생존 형제키 누출)+NONCE 경계-인지화(NEW⊃OLD시 `/proj/innerX` 블라인드 마스킹 조용한 miss)**) | M–L | repo(제네릭; 경로는 인자) |
...
1. **산출물 카운트(SSOT — `doc-reality.sh`의 (C) 체크가 라이브 대조):** `ls core/hooks/*.py core/hooks/*.sh | wc -l` = **29**(`core/hooks`의 .py+.sh 파일; `hook_config.py`·`trust_tier.py` 공용 모듈, `agent-inventory.py` 리컨사일러, `brain-capture.py` 세션 캡처 훅, `verify-observer.py` 검증-관측 훅, `session-tier-observer.py` 세션-티어 옵저버, `loop-write-guard.py` 루프 쓰기가드 포함), `ls core/tests/*.sh | wc -l` = **68**(W-12의 `bootstrap-test.sh`, kiro 레인의 `kiro-preflight-test.sh`, `session-tier-observer-test.sh`, B4의 `grade.sh`·`grade-test.sh`·`loop-ledger-test.sh`·`loop-write-guard-test.sh`, W-2의 `reorg-sync-test.sh` 포함 — `grade.sh`는 루프 도구지만 `core/tests/`에 상주해 카운트됨), `ls agents/*.md | wc -l` = **3**, `ls skills/*/SKILL.md | wc -l` = **10**(brain-ingest·harness-audit·harness-help·manager-audit·persona-review·reorg-sync·spec·supervise·verify-completion·wrap). §3.3 표의 훅·테스트 수치는 2026-07-04 스냅샷이며, 이후 증가분은 이 줄이 정본이다.
