...
| ID | 작업 | 근거 | 완료 조건 (기계 검증) | 규모 |
|---|---|---|---|---|
| P2-1 ✅ 2026-08-10 | `skills/harness-loop/SKILL.md` 작성 — §5의 루프 규정(program.md 상당, 인간만 편집) | §5 | ✅ 실측: 모든 기계 강제는 신설 `core/infra/loop-run.sh`(init/status/attempt/stop) 단일 스크립트에 집중, `skills/harness-loop`·`skills/loop`는 얇은 호출자. `skills/harness-loop/SKILL.md`에 §5.2 9단계를 순서대로 번호 매겨 수록(HUMAN-ONLY-EDITED 마커). 드리프트 게이트 `core/tests/loop-skill-test.sh` 13체크(9단계 순서·load-bearing 토큰 존재·RED 재정렬/단계삭제/토큰누락 픽스처가 실제로 FAIL함을 검증)로 SKILL 프로즈↔메커니즘 정합을 기계 강제. 드라이런 1회로 9단계가 순서대로 로그에 남음을 실측(보고서 첨부) | M |
| P2-2 ✅ 2026-07-11 | `core/tests/grade.sh` 작성 — GATE(sanitize-audit·adapter-parity·hook-config-test·post-commit-autosync·gitleaks) + **L-1 실패모드 체크리스트** → `mode:<id> PASS\|FAIL` + 롤업 `harness_score: X.Y` 마지막 줄 | 루프 요소②⑤ | ✅ 실측: 클린 트리 `harness_score: 10.0`(10 코드가드 모드 PASS + vacuous-parity N/A[GATE-covered — 2026-08-09 운영자 결정: GATE 배터리는 guard map 금지, adapter-parity는 GATE 단일 경로] + review-false-clean N/A — **L-1 개정으로 구 8.0 스칼라 대체**), GATE 실패/타깃위반/루브릭 미파싱 시 `harness_score: 0`(fail-closed=discard, 운영자 선택). 루프 도구라 verify-all에서 제외(재귀 방지), `grade-test.sh` 36체크(hermetic 스텁+모드↔가드 맵 드리프트 게이트+GATE-배터리 map 금지 게이트+untracked 파일 boundary 거부+추출 count-floor) | M |
| P2-3 ✅ 2026-07-11 | 결과 원장 — `core/infra/loop-ledger.sh append`(5열 `commit/harness_score/duration_s/status/description≤80자`, append-only, status enum·수치 검증=무단 강제 금지, .agent/loop/ 자동생성), 브랜치 `harness-loop/<tag>` 규약은 P2-1 SKILL | 루프 요소⑥ | ✅ 실측: keep·discard 행 append 확인, 헤더 1회, 잘못된 status/비수치 거부; witness 사이드카(sha256+행수, 프리픽스 공증 — append-연장은 수용·자기치유, 삭제/절단/재작성만 거부; tamper-evident, High 폐쇄); `loop-ledger-test.sh` 30체크 | S |
| P2-4 ✅ 2026-08-10 | 예산·타임아웃 연동 — `supervisor-goal.sh init` 재사용(세션당 시도 N=5), 런당 10분 타임아웃 kill→`timeout`·discard, GATE 연속 2회 실패 시 circuit-breaker 중단 | 루프 요소④ | ✅ 실측: `loop-run.sh attempt`가 셋 다 강제 — 시도 캡은 `supervisor-goal.sh`의 SQLite `current_wave`/`total_waves`를 SSOT로 재사용(중복 카운터 없음), 타임아웃은 백그라운드 grader + TERM→KILL 워치독(감시 서브셸 자체 fd를 `/dev/null`로 리다이렉트 — 그러지 않으면 취소된 워치독의 orphan `sleep`이 호출자의 stdout 파이프를 잡고 있어 `$(...)` 캡처가 타임아웃 시간만큼 멎는 버그를 실측·수정), circuit-breaker는 연속 `harness_score: 0` 2회에서 `abort circuit-breaker` + active flag 제거. `core/tests/loop-run-test.sh` 37체크(hermetic 임시 git repo, `LOOP_RUN_GRADE_CMD` 스텁): 5회 강제실패→cap 정지+원장 5행, timeout kill 실측(30초 스텁이 1초에 종료), breaker 2회 트립+3번째 시도 거부, resume(새 프로세스 재호출)이 시도 번호를 유지 | S |
...
| W-12 ✅ 2026-08-09 | **부트스트랩 레인** — ① `setup.sh --bootstrap [--dry-run]` 신설: doctor가 WARN만 하던 4종(gitleaks/sqlite3/jq/gh)을 brew/apt로 설치, 패키지별 개별 y/N 동의(AGENT_SETUP_YES 미소비 — 시스템 패키지 설치는 별개 동의축), 비대화형 stdin은 무조건 --dry-run으로 강제 다운그레이드 ② `core/infra/local-layer-export.sh` 신설: 글로벌 레이어(등록 훅·플러그인 캐시·설치 스킬)를 제네릭 매니페스트(`templates/local-layer-manifest.yml.template`)로 내보내되, 렌더링 산출물이 gitleaks 0-findings를 통과해야만 저장(gitleaks 부재도 fail-closed) ③ `docs/getting-started.md` §8 신규 — 새 머신 시나리오(설치→bootstrap→export, import는 미착수 명시) + L3 개인 글루는 프라이빗 dotfiles 레포 권고 | 플러그인 설치만으로는 시스템 의존성이 채워지지 않고, 두 번째 머신에서 첫 머신의 글로벌 레이어 구성을 재현할 방법이 없었음(portability-absorb W2) | ✅ 실측(2026-08-09, PR#109 머지 후 적대적 재리뷰로 HIGH+MED 다수 발견 → PR#111 후속 하드닝): `core/tests/bootstrap-test.sh` 31/31(부트스트랩: 전부-설치됨 1·dry-run 무변이 4·비대화형 강제다운그레이드 2·대화형 y/n 페어링 4·apt 분기 2·미지원 pkg-mgr exit1 1·**tty시임 단독 무인설치 불가** 1 / export: 해피패스 5·**ANTI-VACUOUS RED**(시크릿=스크립트 파일명 → 거부+미저장) 2·gitleaks 부재 fail-closed 2·**config 부재 fail-closed** 1·**인자값 PII/시크릿 미유출** 1·**인자 확장자 미탈취** 1·**extensionless 훅 무손실 가시** 1·**게이트 liveness canary 죽은게이트 차단** 1 / doctor 무영향 2). 보안 게이트 요약: 시크릿 스캔은 렌더 산출물을 gitleaks(repo 룰셋)로 스캔해야만 저장 — 룰셋/스캐너 부재·빈 룰셋·거짓 스텁을 **런타임 canary**(알려진 sk-ant- 토큰을 스캔해 잡히는지 확인)로 fail-close, 훅 emitter는 스크립트 basename만(인자값·PII 미유출), cp 실패·제어문자·--redact 하드닝 + `verify-all.sh`/`doc-reality.sh` 그린 | M | repo |
| W-13 신설 2026-08-09 | **L2 출처·라이선스 원장** — 하네스가 가끔 편입하는 L2 외부 자료(레포 클론·패턴·문서 발췌)의 출처 URL·커밋 핀·라이선스를 단일 원장 문서로 부기 + L2-유래 파일에 원장 항목이 없으면 flag하는 doc-reality류 체크. 카탈로그 구축이 아니라 자기 편입분의 정직한 장부(landscape.md Non-goals와 정합) | portability-absorb W4 벤치마크 제안 — 외부 세팅 조사 중 자기 레포의 L2 편입 추적 부재 확인(landscape.md에 done-condition 스케치가 펜스로 존재) | 원장 문서 실생성 + L2-유래 파일 표본에 항목 존재 + 무원장 픽스처 → flag 검출 | S–M | repo |
| W-14 신설 2026-08-09 | **wayfinder 상태기계 패턴 채택 노트** — supervisor-goal.sh의 wave 상태에 claim(레인 점유자)·blocked-by(차단 사유/의존)·frontier-query(다음 개척점 조회) 3연산 추가 검토. 레인 소유·차단이 상태로 표면화되면 무보고 idle 워커를 기계적으로 식별 가능 | portability-absorb W5 비준(ADOPT-pattern, wayfinder). 같은 캠페인에서 리뷰 레인 무보고 소멸 3연속 실측 — 현재는 레인 상태가 supervisor 컨텍스트에만 존재해 죽음이 무음 | 설계 노트 → 채택 시 sqlite 스키마 확장(claim/blocked_by 컬럼)+status 출력 3필드+배터리 | M | repo |
| W-11 ✅ 2026-07-25 구현·2026-08-09 머지(PR#98) | **플러그인 경로 패리티 + 의존성 doctor** — ① `setup.sh --doctor`에 `gh` CLI 존재 검사(check 19) 신설 ② gitleaks(check 3)·sqlite3+jq(check 14 + `supervisor-goal.sh` 최상단 하드 프리플라이트)의 기존 loud-WARN/human-readable-ERROR 동작을 플러그인-경로 설치 시나리오에서 실측 재검증(재구현 아님 — 이미 존재하던 동작의 증거 공백을 메움) ③ 플러그인 경로에서 brain MCP를 detect-and-guide로 전환(check 20 신설 — `~/.claude.json`의 `mcpServers.brain`을 읽기전용으로 대조해 미등록/오배선 시 opt-in 명령만 안내, 자동쓰기 없음) ④ 플러그인/마켓플레이스 설치 경로를 커버하는 신규 CI 잡 `plugin-path-install` + 로컬 배터리 `core/tests/plugin-path-install-test.sh`(verify-all 동적 발견 편입) | fresh 머신이 `/plugin marketplace add`로만 설치했을 때(`setup.sh --claude` 미실행) doctor의 "claude install path — plugin" 검출(check 17)은 있었지만 그 경로 특유의 갭 3종(gh 부재·brain MCP 미등록·의존성 부재)의 실측 증거는 전무 — shell-install 경로만 clean-install CI(M-5)로 검증돼 있었음(2026-07-25 위임 계약 감사) | ✅ 실측(2026-07-25): doctor 신규 체크 2개(19 gh·20 brain MCP 플러그인경로)가 read-only(`git status --porcelain` 실행 전후 동일 실증) + `setup-doctor-test.sh` 신규 픽스처 9종(o·p~p6, 49→58) + `core/tests/plugin-path-install-test.sh` 13체크(시뮬레이션 플러그인-only 설치 + 무변조 증명 + gitleaks/sqlite3·jq loud-degrade 재확인 + **ANTI-VACUOUS RED 프로브**: 변조된 `.claude-plugin/plugin.json`을 `registry-drift.sh`의 `REGISTRY_DRIFT_ROOT` 시임으로 검출 + 클린 픽스처 컨트롤로 상시-FAIL 아님 실증) + `verify-all.sh` 체크 수 55→56 + ci.yml `plugin-path-install` 신규 잡(clean-install과 별개 job id, YAML lint 통과) | M | repo |
...
| O-2 ✅ 2026-08-10 | **`skills/loop` 범용 반복 실행 스킬** — 반복마다 fresh context, 반복당 정확히 1 task, 진행상태는 파일+git(컨텍스트 비의존 — 재시작 시 이어가기), 하드 예산·시도 캡(`supervisor-goal.sh` 재사용), 머지는 인간 승인. §5 harness-loop의 실행체 겸용(미션이 하네스 자신일 때 = P2, 임의 목표일 때 = 범용) | 루프 레퍼런스 패턴(fresh-context/1-task/파일 상태) + 기존 goal-mode 인프라 재사용 | ✅ 실측: `skills/loop/SKILL.md`(fresh-context·exactly-one-task·resume·human-approval 프로토콜) + `core/infra/loop-run.sh status <slug>`가 상태 json+goal json+원장 tail을 출력 — 프로세스 재기동(새 bash 호출)마다 SQLite `current_wave`로 시도 번호가 이어짐을 `loop-run-test.sh`의 resume 시나리오로 실측(재기동 후 시도2가 `run-<slug>-2.log`로 정확히 이어짐). 캡 도달 시 정지+원장 기록은 P2-4와 동일 메커니즘 공유(중복 구현 없음) | M |
| L-1 ✅ 2026-07-11 (문서분+구현분) | **P2-2 grader 재설계 amendment** — 단일 스칼라 `harness_score` 채점을 실패모드 체크리스트 채점으로 교체: `evals/failure-modes.yaml`(명명된 실패모드 — 적대 리뷰 실적발 건: 조용한 드롭, glob 누락, 우회 플래그, false-CONFIRMED 등)에 대해 모드별 boolean+근거 인용. §5.1 대응표의 해당 행 수정. **E-1(evals/)이 선행 — 역순 금지(grader 없는 루프 = 지표 게이밍 확정)** | "채점 기준이 실패모드를 명시하지 않으면 검증 강화로는 게이밍을 못 막는다"(arxiv.org/pdf/2604.15149) — proxy-hacking 73.8% 실측(openreview.net/forum?id=ikrQWGgxYg) | §5 문서 수정 + `failure-modes.yaml` 초안 ≥8모드 + grade 출력이 모드별 verdict 나열. ✅ 문서분 실측(2026-07-11): `evals/failure-modes.yaml`(schema_version + **12모드** 5필드 완비 — 캠페인 적대 리뷰 실적발 증류, 각 모드에 `caught_in` PR 인용·`detection_signal`·`grader_check` 보울린 질문) + §5.1 `val_bpb` 행을 체크리스트 채점으로 개정(모드별 `mode:<id> PASS\|FAIL — 근거` + 롤업 harness_score 유지로 results.tsv/grep 소비자 계약 무파손) + 배터리 `core/tests/failure-modes-test.sh` **25체크**(파싱·schema_version·≥8모드 floor·유니크 kebab id·전 필드 non-empty + 12모드 존재 대조 + 6 RED 픽스처 mutation검증: too-few/missing/blank/dup/non-kebab/unparseable 전부 거부). ✅ **구현분 실측(2026-07-11)**: `core/tests/grade.sh`가 GATE-pass 경로에서 각 모드를 그 모드를 인코딩한 가드 배터리로 판정(green=hole 닫힘 PASS, red=재개방 FAIL), `mode:<id> PASS\|FAIL — 근거` 나열 + 롤업 `harness_score: X.Y`(PASS수 − 0.5×신뢰불가수), 클린 트리 10.0(vacuous-parity는 GATE-covered N/A — GATE 배터리 map 금지)·GATE실패 0(fail-closed); 모드↔가드 맵=grader 내부 SSOT, `grade-test.sh` 36체크가 드리프트(신규 모드 미매핑·배터리 리네임·GATE 배터리 재유입)까지 게이트 | S(문서 ✅) + M(구현 ✅) |
| L-2 ✅ 2026-07-11 | **grader/tests write-ban + append-only 원장** — 루프 실행 중 개선 에이전트의 `evals/`·`core/tests/` Write/Edit에 `ask`(**deny 아님 — secrets 외 ask-까지 에스컬레이션 원칙 유지**, 캘리브레이션 문서 §3a), results 원장은 append-only(truncate/rewrite에 `ask`) | verifier 게이밍 문서화(테스트 덮어쓰기·채점 함수 변조·감시 코드 사보타주 12% — arxiv.org/pdf/2606.07379) — "TARGET-외 diff discard"의 사전 보강 | ✅ 실측: `core/hooks/loop-write-guard.py`(Write\|Edit\|MultiEdit PreToolUse, hooks.json 배선). 루프 마커(env `AGENT_LOOP_ACTIVE`/플래그파일) 하에서만 활성 — 비루프 세션 완전 inert(오탐 0). `evals/`·`core/tests/` write→ask, 원장 비-append rewrite/edit→ask, 순수 append·TARGET(agents/)·무관 파일→통과. **realpath 봉쇄**로 심링크가 가드 디렉터리로 우회 불가(lexical-containment 교훈 적용). `loop-write-guard-test.sh` 31체크(WHY/FIX 태그·심링크·플래그파일·Bash 쓰기탐지·witness 삭제-재생성 에스컬레이션 포함) | S–M |
...
**2026-07-16 증보**: happytlog "Cost effective harnesses with Fable 5" 기사(+ ClaudeDevs 스레드) 감사 편입 — 정본 증류 `docs/concepts/cost-effective-harnesses.md`. 4개 지침 대조 결과: ①task shape/advisor 패턴=부분(supervise audit-after-wave가 무명명 인스턴스), ②위임 휴리스틱=충족(M-1 사다리), ③조정비용 경제(위임 볼륨이 핸드오프 고정비를 상쇄해야; 소형 작업은 솔로 TOP이 더 저렴 +60% markup 역전 실측)=갭, ④프롬프트 캐시(같은 워커 재사용 vs fresh spawn 컨텍스트 write 재지불)=갭 → M-6~M-8.
...
| M-6 ✅ 2026-07-16 | **비용 하네스 기사 정본 증류** — `docs/concepts/cost-effective-harnesses.md` 신설(3대 지능 배치 패턴·advisor 체크포인트 실측·조정비용 경제·4지침·하네스 매핑 감사표) + `docs/README.md` 색인 편입(동반: 기존 `model-routing.md` 색인 누락 행 동시 수정) | 기사 4지침 중 2개(조정비용·캐시)가 레포 어휘에 전무 — 근거 문서 없이 정책 배선(M-7)만 하면 왜-그런-규칙인지가 유실됨(loop-engineering.md 선례와 동일 구조) | ✅ 실측(2026-07-16): 문서 존재 + README 색인 2행(`model-routing.md`+`concepts/cost-effective-harnesses.md`) + doc-reality green | S |
| M-7 ✅ 2026-07-16 | **정책 배선** — `docs/model-routing.md`에 ⓐIntelligence placement 섹션(orchestrator/advisor/verifier 3배치 + advisor 규칙: 체크포인트는 TOP 유지·mid-run 반복, 단발 upfront 랭킹은 역상관 실측 — 무메커니즘·documented convention 유지) ⓑFloors 2건(조정비용 플로어=when-not-to-delegate, 상대 수치만; 캐시 보존=워커 재사용, verifier fresh-spawn 예외 명시) ⓒenforcement map 정직 라벨링 + supervise SKILL.md Model policy 배치 따름정리 문단·Step 2b 5번째 규칙(Worker reuse) + delegation-contract Wave shaping 2불릿(Handoff must pay for itself·Worker reuse over fresh spawns) | 기사 지침 ③④의 정책 반영 — 특히 fresh-spawn-per-request가 암묵 기본값이었음(캐시 히트율 저하가 저티어 워커의 단가 이점을 상쇄하는 실패 모드) | ✅ 실측(2026-07-16): `grep -q "advisor" docs/model-routing.md` + `grep -qi "cache" skills/supervise/templates/delegation-contract.md` + registry-drift·skill-ab-dataset-test green(frontmatter·Step 2d/4·Hard rules 무수정) | S |
| M-8 | **위임 경제 계측** — model-routing-observer/telemetry-digest 확장: fresh-spawn vs continue(SendMessage 재사용) 비율, wave당 위임 토큰량(경계 대비 페이로드 비) 관측 리포트(WARN-only 관측자 원칙, T-2/P1-5 계열). advisor 체크포인트 도구화(별도 advisor 호출 패턴) 검토 포함 — M-7 관례가 실제로 지켜지는지 측정 없이는 주장 불가(기둥④) | M-7은 관례 문서화까지 — call-time 재사용/볼륨 선택은 정적 검증 불가라 계측이 유일한 대조 수단(모델 라우팅 관측자와 동일 논리) | digest 리포트에 spawn-reuse 비율·위임 볼륨 분포 항목 존재 + 합성 로그 픽스처 검증 | M |
| M-9 | **모델별 하네스 재튜닝 노트** — 하네스 프롬프트·게이트 문구는 모델 교체 시 재검증 대상임을 절차화: 신규 모델을 어느 티어에든 편입하기 전 디스패치 프롬프트 배터리(강조 문구 과민성·지시 문자주의 등 알려진 실패 모드) 스모크를 1회 돌리는 체크리스트 행을 model-routing.md 승격 절차에 추가 | Cursor 스웜 실측(2026-07): 한 프런티어 모델이 강조 문구를 문자 그대로 수용해 통제 불능 악순환 → 실험 중 제외 — 하네스는 모델마다 재튜닝이 필요하다는 외부 실증. 현재 승격 절차(Currency log)는 능력 축만 보고 프롬프트 호환 축이 없음 | model-routing.md 승격 절차에 프롬프트 호환 스모크 행 존재 + fable-5-prompting.md류 모델별 노트가 신규 모델 편입 시 생성되는 관례 명문화 | S |
...
| LE-9 | **fable-5 디스패치 프롬프트 감사 레인** — `concepts/fable-5-prompting.md` 규칙 2–4(anti-wrap-up·근거 인용·경계 명시)를 manager-audit의 검사 레인으로 승격(현재 advisory) | 같은 크로스체크 — 프롬프트 규율이 문서 관례로만 존재. Cursor agent-swarm 실측(플래너 계획 모호 → 워커 토큰 수배 폭증)이 경계 명시 부재가 실비용으로 번지는 사례를 직접 뒷받침 — 감사 레인 승격 근거 보강 | manager-audit.sh 신규 레인 + RED/GREEN 픽스처 | M |
...
### 4.14 X 시리즈 — 외부 하네스 비교 발굴 항목 (2026-07-30 추가)

출처: 2026-07-30 외부 플러그인 `fivetaku/fablize` v2.1.1 전량 정독(40파일 + 이슈 6 + PR 10) 후 이 레포와 1:1 대조. 판정 요지 — 그쪽의 출하 아이디어 3개 중 2개는 여기에 이미 있고 더 강했다(반복실패 공개=`circuit-breaker.py`가 60초 슬라이딩 윈도우로 구현, 완료 판정=Stop 게이트가 주장 텍스트를 읽지 않고 `completion_tests`를 **실행**). 진짜 빈칸 1개(프로젝트 설정 없이도 "이 세션에서 뭐라도 실행했는가")는 이 PR에서 `verify-observer.py`로 폐쇄. **대조 과정에서 이 레포의 실제 결함 1건 발견**: `circuit-breaker.py`의 `exit_code or exitCode`가 성공(`0`)을 falsy로 버려 텍스트 휴리스틱으로 낙하 — 라이브 설치본에 판별 실험 2건으로 실증(`exit 3`+조용한 출력=미기록 / `exit 0`+"0 errors"=실패로 기록), 같은 PR에서 수정. 채택하지 않은 것: 전역 CLAUDE.md 블록 주입 설치(LOCAL-LAYER 매니페스트 규율과 충돌), 텍스트로 pass/fail 추론, 트랜스크립트 스캔 언어 감지.

| ID | 작업 | 근거 | 완료 조건 (기계 검증) | 규모 |
|---|---|---|---|---|
| X-1 | **게이트 holdout arm** — 세션 id 해시로 결정론적 20% OFF 분기 + 결과신호(revert/rework/재지시) 층화 비교로 게이트의 **순효과**를 측정. `telemetry-digest --gates`는 발화율(DEAD/FATIGUE/STALE)만 재고 "껐을 때보다 나은가"에 답하지 못함 — `docs/gate-registry.md` "Is the gate worth its noise?"가 그 질문을 문서화했고 이 항목이 답하는 기계층. **설계 문서 선행 필수**: 비교 대상 프로젝트는 계측 인프라를 만들어놓고 자동 실행 경로를 배선하지 않아 50세션 sunset 판정이 트리거되지 않았다(실측) — 돌지 않는 계측은 부채이므로, 착수 조건은 "자동 실행 경로 + sunset 조건"을 설계에 명시한 뒤 | 리뷰 규율은 문서화됐고 반사실 데이터가 없음 | 설계 문서 + holdout 분기 결정론성 픽스처 + **자동 실행 경로**(훅/CI) + sunset 카운터 | M–L |
| X-2 | **out-of-band 라벨링 원칙 명문화** — 계측 라벨을 모델 컨텍스트(`additionalContext`·`systemMessage`·system 프롬프트)에 주입 금지. in-context 로깅은 측정 대상 자체를 바꾼다(비교 대상 프로젝트 실측: 통제군이 주입된 규칙을 자발 인용) | X-1의 타당성 전제 — 지금도 관측 훅이 나중에 계측을 붙일 때 이 선을 넘기 쉬움 | 원칙 문단 + in-context 라벨 주입 0건 grep 가드 | S |
| X-3 | **circuit-breaker 싱크 계측** — 현재 jsonl 싱크가 없어 gate-registry 등재가 불가(측정 불가 = UNINSTRUMENTED 이하). 이 PR의 픽스 후 잔여 오탐/누락률을 재려면 발화 기록이 필요. exit status 부재라는 런타임 제약도 이 데이터로 재평가 | 이 PR의 픽스가 노출한 갭 — 텍스트 휴리스틱이 유일한 라이브 신호인데 그 정확도가 미측정 | 싱크 + gate-registry 행 + reproduce_test 오염 차단 + 배터리 편입 | S |
...
- **M-6~M-8 편입 (2026-07-16 비용 하네스 기사 감사)**: M-6(정본 증류)·M-7(정책 배선) 문서분은 동일 PR 출하, M-8(위임 경제 계측)은 T-2/P1-5 계측 계열로 적재.

## 7. 이 문서 자체의 검증

커밋 전 수행(전부 레포 루트 기준):

1. **산출물 카운트(SSOT — `doc-reality.sh`의 (C) 체크가 라이브 대조):** `ls core/hooks/*.py core/hooks/*.sh | wc -l` = **30**(`core/hooks`의 .py+.sh 파일; `hook_config.py`·`trust_tier.py` 공용 모듈, `agent-inventory.py` 리컨사일러, `brain-capture.py` 세션 캡처 훅, `verify-observer.py` 검증-관측 훅, `session-tier-observer.py` 세션-티어 옵저버, `loop-write-guard.py` 루프 쓰기가드, `council-escalation-gate.py` council 자동 승격 게이트 포함), `ls core/tests/*.sh | wc -l` = **78**(W-12의 `bootstrap-test.sh`, kiro 레인의 `kiro-preflight-test.sh`, `session-tier-observer-test.sh`, B4의 `grade.sh`·`grade-test.sh`·`loop-ledger-test.sh`·`loop-write-guard-test.sh`, B4 후속의 `gitleaks-allowlist-test.sh`, B5의 `loop-run-test.sh`·`loop-skill-test.sh`, 크로스벤더 워커 레인의 `gemini-preflight-test.sh`·`grok-worker-test.sh`·`antigravity-worker-test.sh`, P1 grounded-completion-gate의 `completion-gate-test.sh`, council 자동 승격의 `council-threshold-test.sh`·`council-escalation-gate-test.sh`, worker-lane 온보딩의 `council-dispatch-path-test.sh`·`worker-setup-skill-test.sh` 포함 — `grade.sh`는 루프 도구지만 `core/tests/`에 상주해 카운트됨), `ls agents/*.md | wc -l` = **3**, `ls skills/*/SKILL.md | wc -l` = **13**(brain-ingest·council-review·harness-audit·harness-help·harness-loop·loop·manager-audit·persona-review·spec·supervise·verify-completion·worker-setup·wrap — B5에서 `harness-loop`·`loop`, 크로스벤더 리뷰에서 `council-review`, worker-lane 온보딩에서 `worker-setup` 추가). §3.3 표의 훅·테스트 수치는 2026-07-04 스냅샷이며, 이후 증가분은 이 줄이 정본이다.
...
4. 백로그 항목 수 검증(2026-07-07 캘리브레이션 배치 갱신, 실측): `grep -cE '^\| P[0-3]-[0-9]+' docs/harness-improvement-plan.md` = **29** (P0 11 + P1 8 + P2 5 + P3 5), 각 행에 완료 조건 존재. H/W 시리즈: `grep -cE '^\| [HW]-[0-9]+' docs/harness-improvement-plan.md` = **18** (H 4 + W 14, W-12 부트스트랩 레인·W-13 L2 원장·W-14 wayfinder 노트 포함). T/E 시리즈: `grep -cE '^\| [TE]-[0-9]+' docs/harness-improvement-plan.md` = **5** (T 4 + E 1 — §4.8). O/L/I 시리즈: `grep -cE '^\| [OLI]-[0-9]+' docs/harness-improvement-plan.md` = **6** (O 2 + L 2 + I 2 — §4.9). M 시리즈: `grep -cE '^\| M-[0-9]+' docs/harness-improvement-plan.md` = **9** (M-1~M-7 ✅ + M-8·M-9 — §4.10). A/G 시리즈: `grep -cE '^\| [AG]-[0-9]+' docs/harness-improvement-plan.md` = **4** (A 2 + G 2; 완료 조건 대신 근거·상태 기재 — §4.6 참고). F 시리즈: `grep -cE '^\| F-[0-9]+' docs/harness-improvement-plan.md` = **2** (F-1·F-2 — §4.11). X 시리즈: `grep -cE '^\| X-[0-9]+' docs/harness-improvement-plan.md` = **4** (X-1~X-4 — §4.14).
...
| 기사: "Cost effective harnesses with Fable 5" (happytlog, 2026-07 — §4.10 M-6~M-8 출처: 지능 배치 3패턴·조정비용·프롬프트 캐시) + ClaudeDevs 스레드(2026-07-08, SWE-bench Pro/BrowseComp 수치) | happytlog.com/2026/07/fable-5-opus-4-8-management-style-ai-cost.html · platform.claude.com (advisor-tool 문서) · github.com/anthropics/claude-cookbooks (managed-agents plan-big-execute-small 노트북) |
