...
| P2-2 ✅ 2026-07-11 | `core/tests/grade.sh` 작성 — GATE(sanitize-audit·adapter-parity·hook-config-test·post-commit-autosync·gitleaks) + **L-1 실패모드 체크리스트** → `mode:<id> PASS\|FAIL` + 롤업 `harness_score: X.Y` 마지막 줄 | 루프 요소②⑤ | ✅ 실측: 클린 트리 `harness_score: 11.0`(11 코드가드 모드 PASS + review-false-clean N/A — **L-1 개정으로 구 8.0 스칼라 대체**), GATE 실패/타깃위반/루브릭 미파싱 시 `harness_score: 0`(fail-closed=discard, 운영자 선택). 루프 도구라 verify-all에서 제외(재귀 방지), `grade-test.sh` 20체크(hermetic 스텁+모드↔가드 맵 드리프트 게이트) | M |
