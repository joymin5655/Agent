...
| P1-5 ✅ 2026-07-05 | 텔레메트리 소비(기둥④ 1단계 재니터) — `core/infra/telemetry-digest.sh`: `supervisor.jsonl` deny/ask 통계 → 규칙 후보 리포트 | 기둥④ | 샘플 로그 입력 → 요약 출력 검증 — ✅ 실측(2026-07-05): action별·specialist별 통계 + `GHOST`(고스트 스페셜리스트)·`NO-ACCEPT`(반복 ask에도 미배차) 2종 규칙후보 산출, 픽스처 스위트 21/21 pass(`32495e6` red 테스트 → `9d9c4ad` 구현 → `79e3452` 리뷰 팔로우업: 비객체 JSON 크래시 가드·file_globs/keywords 메시지 정확도·observe 모드 사각지대 문서화). `agent-harness:code-reviewer` 리뷰 1회전(needs-changes → 반영 후 재검증) | M |
