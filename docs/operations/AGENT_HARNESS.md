...
- 통합 master registry (`internal-platform/.claude/agents/master-registry.json`) 작성
...
| 경로 | 시점 |
|---|---|
| `internal-platform/.claude/agents/master-registry.json` | M1 |
...
5. **Master registry 위치** — 루트 `internal-platform/.claude/` vs Obsidian `raw/docs/operations/agent-registry.md` 미러 → resolved in F2 D-H6 (루트 `.claude/` + Obsidian mirror 둘 다)
