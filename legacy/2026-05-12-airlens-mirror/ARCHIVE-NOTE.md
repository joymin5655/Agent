...
The original skeleton was scoped as an **internal-platform mirror** (per `README.md`: "AirLens agent/runtime mirror repository ... not as a deployable AirLens checkout"). It documented AirLens-specific assets (`Obsidian-airlens/`, `apps/web/`, `claude/rules/root/**`) and only the bootstrap files (`README.md`, `MANIFEST.md`, `setup.sh`, `gitleaks.toml`, `.gitignore`) were written — the planned subdirectories (`claude/`, `codex/`, `docs/`, `github/`, `scripts/`) remained empty.
