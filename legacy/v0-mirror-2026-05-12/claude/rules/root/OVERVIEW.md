...
목적: `.claude/rules/*` 17 file 의 1줄 요약. 자동 로드 컨텍스트 슬림화 (12k → ~1k).
...
- [contributing.md](contributing.md) — 코드 스타일 (TS strict / no `any`, Python PEP 8 + ruff), 함수 50줄 (soft) · **300줄 ceiling = CLAUDE.md / context docs only** (코드 파일은 복잡도 boundary 로 분리), AAA 단위 테스트, 커밋 전 build/lint/test, i18n 6 언어 키 동기, PR 1 기능 단위.
...
- [multi-agent-worktree.md](multi-agent-worktree.md) — claude/codex/gemini 동시 세션 룰 (R1 worktree 강제 · R2 lock · R3 heartbeat · R4 자원 mutex · R4.1 파일 mutex · R5 PR serialize · R6 타 브랜치 침범 금지 · R7 세션 시작 · R7.1 hook stack · R10 untracked 보호 · R11 dashboard · R11.1 main-tree visibility · R12 broadcast · R13 blocked · R14 5 deferral 안전 워크플로우).
...
- [firecrawl-policy.md](policy/firecrawl-policy.md) — Firecrawl MCP 12 tool 화이트리스트 (~75 도메인: 위성 5 + docs 16 + AI/디자인 17 + 갤러리 13 + 마켓 5 + 기술 17 + 학술 2). rate limit (도메인당 50 page/일·plan당 200·세션당 1). 라이선스 frontmatter 의무. wiki 경로 `Obsidian-airlens/wiki/imports/<domain>/<slug>-YYYY-MM-DD.md`.
...
- [humanizer-agent.md](policy/humanizer-agent.md) — `blader/humanizer` MIT 패턴 카탈로그 (Wikipedia "Signs of AI writing") 영어 외부 공개 텍스트 한정 적용. `.claude/agents/copy-humanizer.md` 명시 invoke 만. 5 가드 영역 (i18n JSON / Glass-box / 한국어 / 코드 주석 / 보안 경고) 절대 작용 X.
- [wrap-skill.md](policy/wrap-skill.md) — `/wrap` skill 정책. push 사용자 무조건 / commit + PR 8 영역 자동 (commit msg / git add / git commit / committed broadcast / PR body / gh pr create / pr_opened broadcast / R4.1 file mutex). 5 가드 영역 (production migration / secret / Edge Fn / 결제 / ML uncertainty) 자동화 영원히 X. 화이트리스트 path + gitleaks Layer 1 통과 의무.
- [same-name-skill-priority.md](policy/same-name-skill-priority.md) — 동명 skill 우선순위 매트릭스 (8 skill, hook / Matt Pocock / context-mode / superpowers / addyosmani / gstack 다중 source disambiguation). root `CLAUDE.md §Skill routing` 에서 cross-ref. T+30d (2026-06-05) 데이터 기반 in-place 복원 / context-mode 비활성 / 현 상태 유지 중 택일.
- [wiki-automation-coverage.md](policy/wiki-automation-coverage.md) — wiki life cycle 5 단계 (생성 / 인덱스 / supersede / log rotate / ingest) 가 5 자산 (wiki-synth / wiki-auto-index / wiki-supersede-apply / wiki-log-rotate / firecrawl-wiki-ingest) 으로 이미 자동화. supervisor wiki-curator dispatch 갭은 *인벤토리 무지* 가 원인. 의도별 자산 매핑 안내. 정본 9+1+3 + Obsidian/index.md 는 git tracked 사용자 결정.
...
17 file 중 평균 1-2 file 만 access — 자동 로드 12k → 실 사용 ~2k.
