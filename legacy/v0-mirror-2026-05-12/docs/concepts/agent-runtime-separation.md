...
created: 2026-04-27
updated: 2026-05-08
sources: [.claude/settings.json, .claude/agents, scripts/hooks, scripts/infra, /Users/joymin/.codex/skills]
...
| 구분 | 위치 | 역할 |
|------|------|------|
| Claude agents | `.claude/agents/**` and mirror `claude/agents/root/**` | Claude Code 전용 subagent 정의와 레지스트리 |
| Claude rules | `.claude/rules/**` and mirror `claude/rules/root/**` | Claude Code 런타임 규칙 |
| Claude hooks | `.claude/settings.json`, `.claude/settings.local.json`, `scripts/hooks/**` | Claude Code hook 등록과 실행 스크립트. Local settings는 mirror 제외 |
| Codex skills | `/Users/joymin/.codex/skills/**` and mirror `codex/skills/**` | Codex 전용 skill 지침 |
| 공통 문서 | `docs/**`, AirLens wiki/docs | 설명, 인벤토리, 이관 기록, 운영 결정 |
| 공통 하네스 | `scripts/infra/agent-session.sh`, `scripts/infra/session_store.py`, `scripts/hooks/**` | 세션 lock, heartbeat, R4 mutex, routing, quality gate |

## 현재 검증 결과

- Codex AirLens skill은 13개이며, `airlens-design-director`의 `references/layout-composition-doctrine.md`도 mirror 대상이다.
- `Agent.git`의 `github/workflows/**`는 inactive mirror이므로 이 저장소에서 GitHub Actions로 실행되지 않는다.
- Runtime state (`.claude/locks`, `.claude/logs`, `.claude/settings.local.json`)는 mirror하지 않는다.
- Worktree coordination은 `scripts/infra/agent-session.sh`와 `scripts/hooks/r4-mutex-check.sh`를 기준으로 관리한다.
