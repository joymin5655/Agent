...
Backend abstraction over .claude/locks/active-sessions.json + work-feed.jsonl.
Pattern adapted from LangGraph BaseCheckpointSaver. Anthropic Agent Teams
vocabulary (task_state: pending/in_progress/completed). Swarm Result(...) shape
for handoff event payload. Continue-style schema_version on every event.

Pure stdlib, no external deps. Atomic write via tmp + os.replace + fcntl.flock.

Used by scripts/infra/agent-session.sh (Tier 2 commands: broadcast, dashboard,
peek, update, tail-feed, subscribe). Backward-compatible with existing JSON
schema (new fields are optional).

Plan: ~/.claude/plans/multi-agent-tier-2-coord-plane.md §12.3 (P1 #3).
...
class SessionStore:
    ...
    def __init__(self, root: Path | None = None) -> None:
        ...
        self.lock_dir = self.root / ".claude" / "locks"
    ...
    def list_active(self) -> list[dict[str, Any]]:
        ...
        on read (defense in depth — even if gc lags, dashboard sees correct
        state). Read-only: does not mutate the lock file."""
        return [
    ...
    def gc(self) -> int:
        ...
        Mutates the lock file. Returns the number of entries removed.
        ...
        the worktree-existence layer (B1 plan). The two are complementary —
        callers should run both for full cleanup."""
        data = self._read()
    ...
    # ---------- resource mutex API (R4 compatible) ----------
    ...
    def archive_feed_if_due(self) -> bool:
        """Rotate work-feed.jsonl if ≥ ROTATE_DAYS old. Returns True if rotated."""
        if not self.feed_file.exists():
            return False
        ...
        # If multiple rotations in same month, append-merge to keep one file per month.
    ...
    def peek(self, session_id: str, n: int = 20) -> dict[str, Any]:
        """Return session entry + that session's recent events."""
        return {
    ...
    def dashboard_summary(self) -> str:
        """5-line text summary (B3 plan — token-slim default for SessionStart).

        Format:
          sessions: <N> active (<comma-list of session_id, max 3>)
          locks: <N> shared resources held (<comma-list, max 3>)
          recent: <N> events in last 24h (last: <event_type> @ <ts>)
          events: <last 3 event types arrow-separated>
          uptime: <oldest active session age>
        """
        sessions = self.list_active()
        locks = self._read().get("shared_resource_locks", {})
        recent_all = self.tail_feed(n=20)

        # Filter recent events to last 24h
        ...
        # Line 3 — recent events count + last
        if last_24h:
            last_ev = last_24h[-1]
            l3 = (
...
def make_handoff_event(
    *, from_session: str, to_session: str, intent: str,
    ...
    """Construct a handoff event payload. Pattern: OpenAI Swarm Result(agent=...,
    context_variables=...) — typed, debuggable, peek-special-cased."""
    return {
        "event": "handoff",
...
# ---------- CLI dispatch (called by agent-session.sh) ----------
...
def main(argv: list[str]) -> int:
    ...
    # Python CLI default = json (back-compat for direct callers).
    # agent-session.sh wrapper passes --format summary explicitly (B3 plan)
    # so SessionStart hook gets the token-slim 5-line summary.
    p_dashboard.add_argument(
        "--format", choices=("summary", "json"), default="json",
        help="json (default, full payload) | summary (5-line text, B3 token-slim)",
    )
    sub.add_parser("gc")
